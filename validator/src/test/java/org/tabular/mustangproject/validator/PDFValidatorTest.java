...
import org.tabular.mustangproject.validator.IrrecoverableValidationError;
import org.tabular.mustangproject.validator.PDFValidator;
import org.tabular.mustangproject.validator.ValidationContext;
import org.tabular.mustangproject.validator.XMLValidator;
...
public class PDFValidatorTest extends ResourceCase {
	...
	public void testPDFXMLValidation() {
		 ...
		 * // invalid file here
		 * byte [] contents =
		 * getResourceAsByteArray("attributeBasedXMP_zugferd_2p0_EN16931_Einfach.pdf");
		 *
		 * pv.setFilenameAndContents(
		 * "attributeBasedXMP_zugferd_2p0_EN16931_Einfach.pdf", contents);
		 * pv.validate();
		 ...
		 *
		 * assertEquals(true, pdfvres.contains("valid") &&
		 * !pdfvres.contains("invalid"));
		 * assertEquals(true, xmlvres.contains("invalid"));
		 *
		 * vc.clear();
		 * contents = getResourceAsByteArray("validV1WithAdditionalData.pdf");// need a
		 * more invalid file here
		 *
		 ...
		 * assertEquals(true, pdfvres.contains("valid") &&
		 * !pdfvres.contains("invalid"));
		 * assertEquals(true, xmlvres.contains("valid") &&
		 * !xmlvres.contains("invalid"));
		 * } catch (final IrrecoverableValidationError e) {
		 * // ignore, will be in XML output anyway
		 * }
		 */
