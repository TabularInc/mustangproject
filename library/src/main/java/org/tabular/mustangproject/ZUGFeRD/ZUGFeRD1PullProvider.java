...
public class ZUGFeRD1PullProvider extends ZUGFeRD2PullProvider {
	...
	public void generateXML(IExportableTransaction trans) {
		...
		for (final BigDecimal currentTaxPercent : VATPercentAmountMap.keySet()) {
			...
			if (amount != null) {
				...
				xml += "<ram:ApplicableTradeTax>"
						+ "<ram:CalculatedAmount currencyID=\"" + trans.getCurrency() + "\">"
						+ currencyFormat(amount.getCalculated())
						+ "</ram:CalculatedAmount>" // currencyID=\"EUR\"
						+ "<ram:TypeCode>VAT</ram:TypeCode>"
						+ (displayExemptionReason ? exemptionReason : "")
						+ "<ram:BasisAmount currencyID=\"" + trans.getCurrency() + "\">" + currencyFormat(amount.getBasis())
						+ "</ram:BasisAmount>" // currencyID=\"EUR\"
		...
		xml += "<ram:SpecifiedTradeSettlementMonetarySummation>"
				+ "<ram:LineTotalAmount currencyID=\"" + trans.getCurrency() + "\">" + currencyFormat(calc.getTotal())
				+ "</ram:LineTotalAmount>"
				// currencyID=\"EUR\"
				...
				+ "<ram:GrandTotalAmount currencyID=\"" + trans.getCurrency() + "\">" + currencyFormat(calc.getGrandTotal())
				+ "</ram:GrandTotalAmount>"
				// //
				// currencyID=\"EUR\"
				+ "<ram:TotalPrepaidAmount currencyID=\"" + trans.getCurrency() + "\">" + currencyFormat(calc.getTotalPrepaid())
				+ "</ram:TotalPrepaidAmount>"
				+ "<ram:DuePayableAmount currencyID=\"" + trans.getCurrency() + "\">"
				+ currencyFormat(calc.getGrandTotal().subtract(calc.getTotalPrepaid())) + "</ram:DuePayableAmount>"
				// //
				// currencyID=\"EUR\"
		...
		for (final IZUGFeRDExportableItem currentItem : trans.getZFItems()) {
			...
			xml += "<ram:IncludedSupplyChainTradeLineItem>" +
					...
					+ "</ram:ApplicableTradeTax>"
					+ "<ram:SpecifiedTradeSettlementMonetarySummation>"
					+ "<ram:LineTotalAmount currencyID=\"" + trans.getCurrency() + "\">"
					+ currencyFormat(lc.getItemTotalNetAmount())
					+ "</ram:LineTotalAmount>"
					+ "</ram:SpecifiedTradeSettlementMonetarySummation>";
			if (currentItem.getAdditionalReferences() != null) {
				xml += "<ram:AdditionalReferencedDocument><ram:ID>"
						+ currentItem.getAdditionalReferences()[0].getIssuerAssignedID()
						+ "</ram:ID><ram:TypeCode>130</ram:TypeCode></ram:AdditionalReferencedDocument>";
			} else if (currentItem.getAdditionalReferencedDocumentID() != null) {
				xml += "<ram:AdditionalReferencedDocument><ram:ID>" + currentItem.getAdditionalReferencedDocumentID()
						+ "</ram:ID><ram:TypeCode>130</ram:TypeCode></ram:AdditionalReferencedDocument>";
			}
