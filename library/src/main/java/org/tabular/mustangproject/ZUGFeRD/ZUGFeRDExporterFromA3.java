...
import org.tabular.mustangproject.ZUGFeRD.Version;
...
public class ZUGFeRDExporterFromA3 extends XRExporter implements IZUGFeRDExporter {
	...
	/**
	 * This flag controls whether or not the metadata is overwritten, or kind of
	 * merged.
	 * The merging probably needs to be overhauled, but for my purpose it was good
	 * enough.
	 */
	...
	public ZUGFeRDExporterFromA3 addAdditionalFile(String name, byte[] content) {
		fileAttachments.add(new FileAttachment(name, "text/xml", "Supplement", content)
				.setDescription("ZUGFeRD extension/additional data"));
		return this;
	}
	...
	/***
	 * internal helper: returns the namespace prefix for the given zf/fx version
	 * number
	 *
	...
	/***
	 * internal helper: return the name of the file attachment for the given zf/fx
	 * version
	 *
	 * @param ver     the zf/fx version
	...
	/**
	 * Factur-X is now set by default since ZF 2.1, you have to disable it if you
	 * dont wont it
	...
	/**
	 * Sets a specific XRechnung version from outside. This version needs to be
	 * present in the
	 * meta-data as well. The caller may wish to generate a specific version of
	 * XRechnung
	...
	/***
	 * Generate ZF2.0/2.1 files with filename zugferd-invoice.xml instead of
	 * factur-x.xml
	 */
	...
	protected void prepareDocument() throws IOException {
		...
		// the following three lines are intended to make the pdf more PDF/A conformant
		// if it isn't already
		addSRGBOutputIntend();
		setMarked();
	...
	public IExporter prepare() throws IOException {
		...
		// See documentation ZUGFeRD211_EN/Documentation/ZUGFeRD-2.1.1 -
		// Specification_TA_Part-A.pdf
		...
		for (FileAttachment attachment : fileAttachments) {
			PDFAttachGenericFile(doc, attachment.getFilename(), attachment.getRelation(), attachment.getDescription(),
					attachment.getMimetype(), attachment.getData());
		}
	...
	/**
	 * Sets the producer if the overwrite flag is set or the producer is not already
	 * set.
	 ...
	 * If the overwrite flag is set or no AdobePDFSchema exists in the XMPMetadata,
	...
	/**
	 * Adds a MarkInfo element to the PDF if it doesn't already exist and sets it as
	 * marked.
	 */
