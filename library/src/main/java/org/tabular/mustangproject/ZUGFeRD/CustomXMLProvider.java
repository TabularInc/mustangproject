...
public class CustomXMLProvider implements IXMLProvider {
	...
	public void setXML(byte[] newData) {
		...
		/**
		 * rsm:CrossIndustry is ZF/FX/XR (CII 2016b),rsm:SCRDMCCBDACIOMessageStructure
		 * is Order-X (CIO 2021) and
		...
		if ((!zf.contains("rsm:CrossIndustry")) && (!zf.contains("rsm:SCRDMCCBDACIOMessageStructure"))
				&& (!zf.contains("SCRDMCCBDACIDAMessageStructure"))) {
			throw new RuntimeException(
					"ZUGFeRD XML does not contain <rsm:CrossIndustry, <rsm:SCRDMCCBDACIOMessageStructure or SCRDMCCBDACIDAMessageStructure and can thus not be valid");
		}
