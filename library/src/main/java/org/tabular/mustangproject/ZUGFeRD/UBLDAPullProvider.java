...
public class UBLDAPullProvider implements IXMLProvider {
	...
	public void generateXML(IExportableTransaction trans) {
		...
		if (trans.getReferenceNumber() != null) {
			xml += "<cac:OrderReference>  <cbc:ID>" + XMLTools.encodeXML(trans.getNumber())
		...
		if (trans.getDeliveryDate() != null) {
			xml += "   <cac:Shipment><cbc:ID>1</cbc:ID><cac:Delivery><cbc:ActualDeliveryDate>"
					+ ublDateFormat.format(trans.getDeliveryDate()) + "</cbc:ActualDeliveryDate></cac:Delivery></cac:Shipment>";
		}
		int i = 1;
		for (IZUGFeRDExportableItem item : trans.getZFItems()) {
			xml += "  <cac:DespatchLine>\n" +
					"    <cbc:ID>" + XMLTools.encodeXML(Integer.toString(i++)) + "</cbc:ID>\n" +
					"    <cbc:DeliveredQuantity unitCode=\"" + XMLTools.encodeXML(item.getProduct().getUnit()) + "\">"
					+ item.getQuantity() + "</cbc:DeliveredQuantity>\n" +
