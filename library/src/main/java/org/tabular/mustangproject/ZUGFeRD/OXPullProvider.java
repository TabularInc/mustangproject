...
import static org.tabular.mustangproject.ZUGFeRD.model.DocumentCodeTypeConstants.CORRECTEDINVOICE;
...
import org.tabular.mustangproject.EStandard;
...
public class OXPullProvider extends ZUGFeRD2PullProvider {
	...
	public void generateXML(IExportableTransaction trans) {
		...
		if ((paymentTermsDescription == null) && (trans.getDocumentCode() != CORRECTEDINVOICE)/*
		...
		for (final IZUGFeRDExportableItem currentItem : trans.getZFItems()) {
			...
			if (currentItem.getProduct().getTaxExemptionReason() != null) {
				...
				// XMLTools.encodeXML(currentItem.getProduct().getTaxExemptionReason()) +
				// "</ram:ExemptionReason>";
			}
			 ...
			 * for (IReferencedDocument currentReferencedDocument :
			 * currentItem.getReferencedDocuments()) {
		...
		xml += "</ram:ApplicableHeaderTradeDelivery>\n<ram:ApplicableHeaderTradeSettlement>"
				// + "<ram:PaymentReference>" + XMLTools.encodeXML(trans.getNumber()) +
		...
		if (trans.getDocumentCode() != null) {
			if ((trans.getDocumentCode().equals(CORRECTEDINVOICE))/*
																														 * ||(trans.getDocumentCode().equals
																														 * (DocumentCodeTypeConstants.CREDITNOTE))
																														 */) {
				hasDueDate = false;
			}
		}
		...
		for (final BigDecimal currentTaxPercent : VATPercentAmountMap.keySet()) {
			...
			if (amount != null) {
				 ...
				 * final boolean displayExemptionReason =
				 * CATEGORY_CODES_WITH_EXEMPTION_REASON.contains(amountCategoryCode);
				 * xml += "<ram:ApplicableTradeTax>\n"
				 * + "<ram:CalculatedAmount>" + currencyFormat(amount.getCalculated())
				 * + "</ram:CalculatedAmount>\n" //currencyID=\"EUR\"
				 * + "<ram:TypeCode>VAT</ram:TypeCode>\n"
				 * + (displayExemptionReason ? exemptionReason : "")
				 * + "<ram:BasisAmount>" + currencyFormat(amount.getBasis()) +
				 * "</ram:BasisAmount>\n" // currencyID=\"EUR\"
		...
		xml += "<ram:SpecifiedTradeSettlementHeaderMonetarySummation>"
				...
				// //
				// currencyID=\"EUR\"
				// + "<ram:TotalPrepaidAmount>" + currencyFormat(calc.getTotalPrepaid()) +
