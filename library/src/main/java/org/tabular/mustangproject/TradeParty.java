package org.tabular.mustangproject;
...
import org.tabular.mustangproject.ZUGFeRD.IZUGFeRDExportableContact;
import org.tabular.mustangproject.ZUGFeRD.IZUGFeRDExportableTradeParty;
import org.tabular.mustangproject.ZUGFeRD.IZUGFeRDLegalOrganisation;
import org.tabular.mustangproject.ZUGFeRD.IZUGFeRDTradeSettlement;
...
public class TradeParty implements IZUGFeRDExportableTradeParty {
				...
				if (currentItemNode.getLocalName() != null) {
						...
						for (int partyIndex = 0; partyIndex < party.getLength(); partyIndex++) {
							if (party.item(partyIndex).getLocalName() != null) {
								...
								if (currentTopElementName.equals("PartyTaxScheme")) {
									...
									for (int partyTaxSchemeIndex = 0; partyTaxSchemeIndex < partyTaxScheme
											.getLength(); partyTaxSchemeIndex++) {
								...
								// if (currentTopElementName.equals("PartyTaxScheme")) {
								// NodeList partyTaxScheme = party.item(partyIndex).getChildNodes();
								// for (int partyTaxSchemeIndex = 0; partyTaxSchemeIndex <
								// partyTaxScheme.getLength(); partyTaxSchemeIndex++) {
								// if (partyTaxScheme.item(partyTaxSchemeIndex).getLocalName() != null) {
								// if
								// (partyTaxScheme.item(partyTaxSchemeIndex).getLocalName().equals("TaxScheme"))
								// {
								// NodeList taxScheme =
								// partyTaxScheme.item(partyTaxSchemeIndex).getChildNodes();
								// for (int taxSchemeIndex = 0 ; taxSchemeIndex < taxScheme.getLength();
								// taxSchemeIndex++) {
								// if (taxScheme.item(taxSchemeIndex).getLocalName() != null) {
								// if(taxScheme.item(taxSchemeIndex).getLocalName().equals("ID")){
								// if
								// (partyTaxScheme.item(partyTaxSchemeIndex).getLocalName().equals("CompanyID"))
								// {
								// setTaxID(partyTaxScheme.item(partyTaxSchemeIndex).getTextContent());
								// } else {
								...
								// }
								/*
								 * UBL only: formally it can have a name as well but BT27 party name *should* be
								 * stored in
								...
								if (currentTopElementName.equals("PostalAddress")) {
									...
									for (int postalChildIndex = 0; postalChildIndex < postal.getLength(); postalChildIndex++) {
										if (postal.item(postalChildIndex).getLocalName() != null) {
											...
											// unknow correspondence if
											// (postal.item(postalChildIndex).getLocalName().equals("LineThree")) {
					...
					if (currentUBLChild.equals("GlobalID")) {
						if (nodes.item(nodeIndex).getAttributes().getNamedItem("schemeID") != null) {
							SchemedID gid = new SchemedID()
									.setScheme(nodes.item(nodeIndex).getAttributes().getNamedItem("schemeID").getNodeValue())
									.setId(nodes.item(nodeIndex).getTextContent());
							addGlobalID(gid);
						}
								...
								if ((taxChilds.item(taxChildIndex).getLocalName().equals("TaxScheme"))) {
									if (taxChilds.item(taxChildIndex).getLocalName().equals("CompanyID")) {
										if (taxChilds.item(taxChildIndex).getLocalName().equals("ID")) {
											...
											// setTaxID(partyTaxScheme.item(partyTaxSchemeIndex).getTextContent());
											if (taxChilds.item(taxChildIndex).getAttributes().getNamedItem("ID").getNodeValue()
													.equals("FC")) {
												if (taxChilds.item(taxChildIndex).getLocalName().equals("CompanyID")) {
					...
					if (itemChilds.item(itemChildIndex).getLocalName() != null) {
						...
						if (itemChilds.item(itemChildIndex).getLocalName().equals("GlobalID")) {
							if (itemChilds.item(itemChildIndex).getAttributes().getNamedItem("schemeID") != null) {
								SchemedID gid = new SchemedID()
										.setScheme(itemChilds.item(itemChildIndex).getAttributes().getNamedItem("schemeID").getNodeValue())
										.setId(itemChilds.item(itemChildIndex).getTextContent());
								addGlobalID(gid);
							}
						}
						...
						if (itemChilds.item(itemChildIndex).getLocalName().equals("URIUniversalCommunication")) {
							...
							for (int URIChildIndex = 0; URIChildIndex < URIchilds.getLength(); URIChildIndex++) {
								...
								if ((currentNode.getLocalName() != null) && (currentNode.getLocalName().equals("URIID")
										...
										&& (URIchilds.item(URIChildIndex).getAttributes().getNamedItem("schemeID").getNodeValue()
												.equals("EM"))) {
								...
								if (taxChilds.item(taxChildIndex).getLocalName() != null) {
									if ((taxChilds.item(taxChildIndex).getLocalName().equals("ID"))) {
										if (taxChilds.item(taxChildIndex).getAttributes().getNamedItem("schemeID") != null) {
											...
											if (firstChild != null) {
												if (taxChilds.item(taxChildIndex).getAttributes().getNamedItem("schemeID").getNodeValue()
														.equals("VA")) {
	 ...
	 * (while setEmail has to be defined here getEmail comes from
	...
	/***
	 * required, usually done in the constructor: the complete name of the
	 * organisation
	 * 
	...
	/***
	 * usually set in the constructor, required for recipients in german invoices:
	 * postcode
	 * 
	 * @param zip postcode
	 * @return fluent setter
	...
	/***
	 * usually set in constructor, usually required in germany, the city of the
	 * organisation
	 * 
	 * @param location city
	 * @return fluent setter
	...
	public IZUGFeRDTradeSettlement[] getAsTradeSettlement() {
		...
		List<IZUGFeRDTradeSettlement> tradeSettlements = Stream.concat(bankDetails.stream(), debitDetails.stream())
				.map(IZUGFeRDTradeSettlement.class::cast).collect(Collectors.toList());
	...
	/***
	 * Sets two parts of additional address at once, e.g. which building and which
	 * floor
	 * (if building is not in question which floor can also be set with the first
	 * part only :-))
	 *
	 * @param additionalAddress1 first part of additional info, e.g. "Rear building"
	 * @param additionalAddress2 second part of additional address info, e.g. "2nd
	 *                           floor"
	 * @return fluent setter
	 */
	 ...
	 * e.g. which floor (if LineTwo=setAdditionalAddress has already been used e.g.
	 * for which building)
	 * 
	 * @return lineThree
	 */
