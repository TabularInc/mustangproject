...
import org.tabular.mustangproject.ZUGFeRD.ZUGFeRDDateFormat;
...
public class XMLTools extends XMLWriter {
	 ...
	 * @return a BigDecimal with the value provides as String or a BigDecimal with
	 *         value 0.00 if an error occurs
	 */
	...
	/***
	 * formats a number so that at least minDecimals are displayed but at the
	 * maximum maxDecimals are there, i.e.
	...
	public static String nDigitFormatDecimalRange(BigDecimal value, int maxDecimals, int minDecimals) {
		...
		while ((curDecimals > minDecimals) && (value.setScale(curDecimals, RoundingMode.HALF_UP)
				.compareTo(value.setScale(curDecimals - 1, RoundingMode.HALF_UP)) == 0)) {
			curDecimals--;
		}
