...
public class MustangReaderWriterCustomXMLTest extends TestCase {
	...
	public void testCustomZF2Export() {
		...
		try {
			...
			IZUGFeRDExporter zea1 = new ZUGFeRDExporterFromA1().setProducer("My Application").setCreator("Test")
					.setProfile(Profiles.getByName("EN16931"))
					.load(SOURCE_PDF);

			final byte[] UTF8ByteOrderMark = new byte[] { (byte) 0xef, (byte) 0xbb, (byte) 0xbf };
			/*
			 * we have much more information than just in the basic profile (comfort or
			 * extended) but it's perfectly valid to provide more information, just not
			 * less.
			 * test the export with bom (which is valid) because this will also test the
			 * import (which also needs to work and needs to be tested)
			 */
