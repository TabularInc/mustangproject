...
public class UXTest extends TestCase {
	...
	public void testShortExport() {
		 ...
		 * Date()).setDeliveryDate(new
		 * Date()).setOwnOrganisationName("My company").setOwnStreet("teststr").
		 * setOwnZIP("12345").setOwnLocation("teststadt").setOwnCountry("DE").
		 * setOwnTaxID("4711").setOwnVATID("0815").setRecipient(new
		 * Contact("Franz Müller", "0177123456", "fmueller@test.com", "teststr.12",
		 * "55232", "Entenhausen", "DE")).setNumber("INV/123").addItem(new Item(new
		 * Product("Testprodukt", "", "C62", BigDecimal.ZERO), new BigDecimal(1.0), new
		 * BigDecimal(1.0)));
		 * new ZUGFeRDExporterFromA1().load("source.pdf").setTransaction(i).export(
		 * TARGET_PDF);
		 * } catch (Exception e) {
		 ...
		 * assertEquals(zi.getBuyerTradePartyAddress().getCountrySubDivisionName(),
		 * null);
		 ...
		 * assertEquals(zi.getSellerTradePartyAddress().getCountrySubDivisionName(),
		 * null);
		 ...
		 * 
		 * List<org.tabular.mustangproject.Item> li = zi.getLineItemList();
		 ...
		 * "");
		 * assertEquals(zi.getLineItemList().get(0).getLineTotalAmount().toString(),
		 * "160.00");
		 * assertEquals(zi.getLineItemList().get(0).getQuantity().toString(), "1.0000");
		 * assertEquals(zi.getLineItemList().get(0).getGrossPrice().toString(),
		 * "160.0000");
		 * assertEquals(zi.getLineItemList().get(0).getProduct().getVATPercent().
		 * toString(), "7.00");
		 * assertEquals(zi.getLineItemList().get(0).getProduct().getName(),
