...
import org.tabular.mustangproject.ZUGFeRD.model.DocumentContextParameterTypeConstants;
...
public class MustangReaderWriterTest extends MustangReaderTestCase {
	 ...
	 * values. As only Name Ascending is supported for Test Unit sequence, I renamed
	 * the this testAImport
	...
	public void testZExport() {
		...
		try (InputStream SOURCE_PDF = this.getClass()
				...
				IZUGFeRDExporter ze = new ZUGFeRDExporterFromA1().setZUGFeRDVersion(2).setProfile(Profiles.getByName("EN16931"))
						.load(SOURCE_PDF)) {
			...
			// assertFalse(pdfContent.indexOf("<zf:ConformanceLevel>EN
			// 16931</zf:ConformanceLevel>") == -1);
			// assertFalse(pdfContent.indexOf("<pdfaSchema:prefix>zf</pdfaSchema:prefix>")
			// == -1);
			// assertFalse(pdfContent.indexOf("urn:zugferd:pdfa:CrossIndustryDocument:invoice:2p0#")
			// == -1);
	...
	public void testZExportv1Profile() {
		...
		try (InputStream SOURCE_PDF = this.getClass()
				...
				IZUGFeRDExporter ze = new ZUGFeRDExporterFromA1().setZUGFeRDVersion(1)
						.setProfile(Profiles.getByName("COMFORT", 1)).load(SOURCE_PDF)) {
	...
	public void testFXExport() throws Exception {
		...
		try (InputStream SOURCE_PDF = this.getClass()
				...
				ZUGFeRDExporterFromA1 ze = new ZUGFeRDExporterFromA1().setZUGFeRDVersion(2)
						.setProfile(Profiles.getByName("EN16931")).load(SOURCE_PDF)) {
