...
import org.tabular.mustangproject.ZUGFeRD.LineCalculator;
import org.tabular.mustangproject.ZUGFeRD.TransactionCalculator;
...
public class CalculationTest {
	...
	public void testLineCalculatorInclusiveAllowance() {
		// This test failed with previous implementation. By rounding the totalVATAmount
		// to 2 decimal places the result became wrong
		final IZUGFeRDExportableProduct product = new IZUGFeRDExportableProductImpl().setVatPercent(valueOf(16));
	...
	public void testTotalCalculatorGrandTotalRounding() {
		...
		try {
			...
			invoice.setDetailedDeliveryPeriod(sqlDate.parse("2020-12-01 - 2020-12-31".split(" - ")[0]),
					sqlDate.parse("2020-12-01 - 2020-12-31".split(" - ")[1]));
		...
		TradeParty recipient = new TradeParty("Teston GmbH" + " " + "Zentrale" + " " + "", "Testweg 5", "11111", "Testung",
				"DE");
		...
		if (item_increase.compareTo(BigDecimal.ZERO) > 0) {
			item.addCharge(new Charge().setPercent(item_increase).setTaxPercent(sales_tax_percent1).setReasonCode("ZZZ")
					.setReason("Zuschlag"));
		}
		if (item_discount.compareTo(BigDecimal.ZERO) > 0) {
			item.addAllowance(new Allowance().setPercent(item_discount).setTaxPercent(sales_tax_percent1).setReasonCode("95")
					.setReason("Rabatt"));
		}
		...
		if (total_increase_percent.compareTo(BigDecimal.ZERO) > 0) {
			invoice.addCharge(new Charge().setPercent(total_increase_percent).setTaxPercent(sales_tax_percent1)
					.setReasonCode("ZZZ").setReason("Zuschläge"));
		}
		if (total_discount_percent.compareTo(BigDecimal.ZERO) > 0) {
			invoice.addAllowance(new Allowance().setPercent(total_discount_percent).setTaxPercent(sales_tax_percent1)
					.setReasonCode("95").setReason("Rabatte"));
		}
	...
	/**
	 * LineCalculator should not throw an exception when calculating a
