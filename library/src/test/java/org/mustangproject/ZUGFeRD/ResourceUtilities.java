...
/**
 * This test utility class is providing usability functions to test file
 * resources
 */
public class ResourceUtilities {
	...
	public static void saveFile(Charset encoding, String path, String content) throws FileNotFoundException {
		...
		try (PrintWriter out = new PrintWriter(new OutputStreamWriter(new FileOutputStream(new File(path)), encoding),
				true)) {
			out.println(content);
		}
	}
	...
	/**
	 * The relative path of the test file will be resolved and the absolute will be
	 * returned
	 *
	 ...
	 * @throws URISyntaxException if no URI could be created from the given relative
	 *                            path
	 */
	...
	/**
	 * Relative to the test output directory a test file will be returned dependent
	 * on the relativeFilePath provided.
	 *
	...
	public static String toExternalForm(URI u) {
		...
		if (u.isOpaque()) {
			...
			if (u.getHost() != null) {
				...
				boolean needBrackets = ((u.getHost().indexOf(':') >= 0) && !u.getHost().startsWith("[")
						&& !u.getHost().endsWith("]"));
				if (needBrackets)
					sb.append('[');
				sb.append(u.getHost());
